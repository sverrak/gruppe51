...
public class Room {
	...
	public Boolean addEventToRoom(Event event){
		...
		//	System.out.println(event.getTitle() + ", was added to: " + this.getName());
			//Hvordan sortere roomSchedule p√• en hensiktsmessig maate?
			//roomSchedule.sort(c);
			return true;
		}
		return false;	
...
/*	
	...
	*/
	//bruker samme isAvailable som i Employee-klassen da denne ser ut til Â fungere
	public boolean isAvailable(Date startTime, Date endTime){
		if(this.roomSchedule.size() == 0){
			return true;
		...
		for (int i = 0; i < roomSchedule.size()-1; i++) {
			if(roomSchedule.get(i).getEndTime().compareTo(startTime) < 0 && endTime.compareTo(roomSchedule.get(i+1).getStartTime()) < 0){ // fortegn her virker galt. Motsatt tegn fÔøΩr hele dritten til ÔøΩ henge, men det skyldes kanskje feil i compareTo
				return true;
			}
		}
