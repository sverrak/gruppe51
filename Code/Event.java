...
public class Event implements Comparable<Event>{
	...
	// M√• ogs√• sjekke om det er ledige rom p√• tidspunktet
	...
	public void setDescription(String description) {
		// For eksempel. Bare for at man ikke skal v√¶re helt dust og sette dritlang description
	...
	// Skal vi ha st√∏tte for dette? M√• bare kalles med rom man VET er ledige, ref findLocation() i kalender-klassen
	...
	public String getYear(){		// det du har gjort her med loCaleToString() osv fungerer ikke. NÔøΩ klarer jeg ikke skrive ut Events. FÔøΩr NullPointerException :(
	...
	@Override
	public int compareTo(Event event) {			// mistenker at det blir feil her. Kan vÊre problem med Â sammenlikne mÂneder (de er jo ikke tall med denne implementasjonen)  
			...
			if(this.getMonth().equals(event.getMonth())){
				if(this.getDay().equals(event.getDay())){
					if(this.getHour().equals(event.getHour())){
						if(this.getMinute().equals(event.getMinute())){			// blÂser i sekundforskjeller. Tidspunkt for events angis uansett ikke med mer enn minuttspresisjon
							return 0;
